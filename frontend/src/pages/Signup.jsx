...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(false);
    if (ok) navigate("/");
    else setError("Could not create account. Email may already be in use.");
  };
      ...
      <div className="w-full max-w-sm bg-white border border-slate-200 rounded-2xl shadow-sm p-6 sm:p-8">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 rounded-lg text-sm disabled:opacity-60 transition-colors"
          >
